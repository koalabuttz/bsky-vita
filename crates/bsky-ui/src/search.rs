//! Search screen — top-level.
//!
//! Reachable via the tab bar's Search tab. IME for the query string;
//! D-pad LEFT/RIGHT toggles between People and Posts modes. Results
//! render either as compact actor rows (avatar + name + handle) or as
//! full timeline-style post rows. Tap an actor → their profile; tap a
//! post → its thread.

use std::collections::HashSet;
use std::sync::Arc;

use atrium_api::app::bsky::actor::defs::ProfileView;
use atrium_api::app::bsky::feed::defs::{FeedViewPost, FeedViewPostData, PostView};
use bsky_auth::AuthClient;
use bsky_ime::{Ime, ImeMode, ImeState, TextBoxMode};
use bsky_input::buttons;
use bsky_render::{theme, Color, Font, Frame, SCREEN_HEIGHT, SCREEN_WIDTH};
use bsky_worker::{WorkRequest, WorkResponse};

use crate::cdn::avatar_thumbnail_jpeg;
use crate::profile::ProfileScreen;
use crate::screen::{Screen, ScreenAction};
use crate::tabbar::{TabBar, TopLevel, TAB_BAR_HEIGHT};
use crate::thread::ThreadScreen;
use crate::timeline::{
    draw_post_row, measure_post_row, AVATAR_SIZE, ROW_PAD_X, ROW_PAD_Y, TEXT_LEFT,
};
use crate::widget::{ButtonState, Rect, UiCtx};

const HEADER_H: i32 = 40;
const QUERY_BAR_H: i32 = 56;
const MODE_BAR_H: i32 = 36;
const RESULTS_TOP: i32 = HEADER_H + QUERY_BAR_H + MODE_BAR_H;
const VIEWPORT_BOTTOM: i32 = SCREEN_HEIGHT - TAB_BAR_HEIGHT;
const VIEWPORT_H: i32 = VIEWPORT_BOTTOM - RESULTS_TOP;

const ACTOR_ROW_H: i32 = ROW_PAD_Y * 2 + AVATAR_SIZE;

#[derive(Copy, Clone, PartialEq, Eq)]
enum Mode {
    People,
    Posts,
}

enum SearchState {
    Idle, // no query yet
    LoadingActors,
    LoadingPosts,
    Actors(Vec<ProfileView>),
    Posts(Vec<FeedViewPost>),
    Error(String),
}

pub struct SearchScreen {
    client: Arc<AuthClient>,
    query: String,
    mode: Mode,
    state: SearchState,
    selected_idx: usize,
    scroll_y: f32,
    row_heights: Vec<i32>,
    inflight_avatars: HashSet<String>,
    query_btn: ButtonState,
    people_btn: ButtonState,
    posts_btn: ButtonState,
    tab_bar: TabBar,
}

impl SearchScreen {
    pub fn new(client: Arc<AuthClient>) -> Self {
        Self {
            client,
            query: String::new(),
            mode: Mode::People,
            state: SearchState::Idle,
            selected_idx: 0,
            scroll_y: 0.0,
            row_heights: Vec::new(),
            inflight_avatars: HashSet::new(),
            query_btn: ButtonState::default(),
            people_btn: ButtonState::default(),
            posts_btn: ButtonState::default(),
            tab_bar: TabBar::new(TopLevel::Search),
        }
    }

    fn dispatch_search(&mut self, ctx: &UiCtx) {
        let Some(worker) = ctx.worker else { return };
        if self.query.trim().is_empty() {
            self.state = SearchState::Idle;
            return;
        }
        self.row_heights.clear();
        self.selected_idx = 0;
        self.scroll_y = 0.0;
        match self.mode {
            Mode::People => {
                self.state = SearchState::LoadingActors;
                worker.send(WorkRequest::SearchActors {
                    q: self.query.clone(),
                    cursor: None,
                });
            }
            Mode::Posts => {
                self.state = SearchState::LoadingPosts;
                worker.send(WorkRequest::SearchPosts {
                    q: self.query.clone(),
                    cursor: None,
                });
            }
        }
    }
}

impl Screen for SearchScreen {
    fn frame(
        &mut self,
        frame: &mut Frame,
        font: &Font,
        ctx: &UiCtx,
        ime: &mut Ime,
    ) -> ScreenAction {
        // IME drain into the query buffer + auto-search on Finish.
        let mut ime_finished = false;
        match ime.poll() {
            ImeState::Finished(s) => {
                self.query = s;
                ime.close();
                ime_finished = true;
            }
            ImeState::Cancelled | ImeState::Aborted => {
                ime.close();
            }
            _ => {}
        }
        if ime_finished {
            self.dispatch_search(ctx);
        }

        // Mode toggle: d-pad LEFT/RIGHT cycles, or tap on a People/Posts
        // tab in the mode bar (handled in the mode-bar render below).
        // Both paths funnel through `requested_mode`; we apply at the end.
        let interactive = !ime.is_active();
        let mut requested_mode: Option<Mode> = None;
        if interactive
            && (ctx.pad.just_pressed(buttons::LEFT) || ctx.pad.just_pressed(buttons::RIGHT))
        {
            requested_mode = Some(match self.mode {
                Mode::People => Mode::Posts,
                Mode::Posts => Mode::People,
            });
        }

        // Selection nav within results.
        let count = match &self.state {
            SearchState::Actors(v) => v.len(),
            SearchState::Posts(v) => v.len(),
            _ => 0,
        };
        let mut selection_changed = false;
        if count > 0
            && interactive
            && (ctx.pad.just_pressed(buttons::UP) || ctx.pad.just_pressed(buttons::DOWN))
        {
            if ctx.pad.just_pressed(buttons::UP) && self.selected_idx > 0 {
                self.selected_idx -= 1;
                selection_changed = true;
            }
            if ctx.pad.just_pressed(buttons::DOWN) && self.selected_idx + 1 < count {
                self.selected_idx += 1;
                selection_changed = true;
            }
        }

        // Lazy row-height measure.
        match &self.state {
            SearchState::Actors(actors) => {
                while self.row_heights.len() < actors.len() {
                    self.row_heights.push(ACTOR_ROW_H);
                }
            }
            SearchState::Posts(posts) => {
                while self.row_heights.len() < posts.len() {
                    let i = self.row_heights.len();
                    let h = measure_post_row(frame, font, &posts[i], ctx.emoji, false);
                    self.row_heights.push(h);
                }
            }
            _ => {}
        }

        // Layout + auto-scroll on selection.
        let total_h: i32 = self.row_heights.iter().sum();
        let max_scroll = (total_h - VIEWPORT_H).max(0) as f32;
        if selection_changed && self.selected_idx < self.row_heights.len() {
            let row_top: i32 = self.row_heights[..self.selected_idx].iter().sum();
            let row_h = self.row_heights[self.selected_idx];
            let view_top = self.scroll_y as i32;
            let view_bottom = view_top + VIEWPORT_H;
            const SCROLL_MARGIN: i32 = 50;
            if row_h > VIEWPORT_H {
                // Taller-than-viewport rows pin to the top.
                self.scroll_y = (row_top - SCROLL_MARGIN).max(0) as f32;
            } else if row_top < view_top + SCROLL_MARGIN {
                self.scroll_y = (row_top - SCROLL_MARGIN).max(0) as f32;
            } else if row_top + row_h > view_bottom - SCROLL_MARGIN {
                self.scroll_y =
                    (row_top + row_h + SCROLL_MARGIN - VIEWPORT_H).max(0) as f32;
            }
        }
        let stick_y = ctx.pad.left_stick.1;
        let mag = stick_y.unsigned_abs() as f32;
        const STICK_DEADZONE: f32 = 32.0;
        if mag > STICK_DEADZONE {
            let sign: f32 = if stick_y < 0 { -1.0 } else { 1.0 };
            let effective = (mag - STICK_DEADZONE) * sign;
            self.scroll_y += effective / 24.0;
        }
        self.scroll_y = self.scroll_y.clamp(0.0, max_scroll);

        // Avatar dispatch for visible result rows.
        if let Some(worker) = ctx.worker {
            let mut y_probe = RESULTS_TOP - self.scroll_y as i32;
            let avatars: Vec<Option<&str>> = match &self.state {
                SearchState::Actors(v) => {
                    v.iter().map(|a| a.avatar.as_deref()).collect()
                }
                SearchState::Posts(v) => v
                    .iter()
                    .map(|p| p.post.author.avatar.as_deref())
                    .collect(),
                _ => Vec::new(),
            };
            for (avatar, &row_h) in avatars.iter().zip(self.row_heights.iter()) {
                let row_bottom = y_probe + row_h;
                if row_bottom > RESULTS_TOP && y_probe < VIEWPORT_BOTTOM {
                    if let Some(url) = avatar {
                        let url = avatar_thumbnail_jpeg(url);
                        if !ctx.texture_cache.contains(&url)
                            && !self.inflight_avatars.contains(&url)
                        {
                            worker.send(WorkRequest::FetchImage { url: url.clone() });
                            self.inflight_avatars.insert(url);
                        }
                    }
                }
                y_probe += row_h;
            }
        }

        // Render results.
        match &self.state {
            SearchState::Idle => {
                frame.draw_text_centered(
                    font,
                    SCREEN_HEIGHT / 2,
                    theme::TEXT_MUTED,
                    1.0,
                    "Tap the search box and enter a query",
                );
            }
            SearchState::LoadingActors | SearchState::LoadingPosts => {
                frame.draw_text_centered(
                    font,
                    SCREEN_HEIGHT / 2,
                    theme::TEXT_MUTED,
                    1.0,
                    "Searching…",
                );
            }
            SearchState::Actors(actors) => {
                if actors.is_empty() {
                    frame.draw_text_centered(
                        font,
                        SCREEN_HEIGHT / 2,
                        theme::TEXT_MUTED,
                        1.0,
                        "No people found",
                    );
                } else {
                    let mut y = RESULTS_TOP - self.scroll_y as i32;
                    for (i, (actor, &row_h)) in
                        actors.iter().zip(self.row_heights.iter()).enumerate()
                    {
                        let row_bottom = y + row_h;
                        if row_bottom > RESULTS_TOP && y < VIEWPORT_BOTTOM {
                            draw_actor_row(
                                frame,
                                font,
                                actor,
                                y,
                                row_h,
                                ctx,
                                i == self.selected_idx,
                            );
                        }
                        y += row_h;
                    }
                }
            }
            SearchState::Posts(posts) => {
                if posts.is_empty() {
                    frame.draw_text_centered(
                        font,
                        SCREEN_HEIGHT / 2,
                        theme::TEXT_MUTED,
                        1.0,
                        "No posts found",
                    );
                } else {
                    let mut y = RESULTS_TOP - self.scroll_y as i32;
                    for (i, (post, &row_h)) in
                        posts.iter().zip(self.row_heights.iter()).enumerate()
                    {
                        let row_bottom = y + row_h;
                        if row_bottom > RESULTS_TOP && y < VIEWPORT_BOTTOM {
                            draw_post_row(
                                frame,
                                font,
                                post,
                                y,
                                row_h,
                                ctx.emoji,
                                ctx.texture_cache,
                                ctx.avatar_mask,
                                ctx.avatar_mask_field,
                                i == self.selected_idx,
                                false,
                            );
                        }
                        y += row_h;
                    }
                }
            }
            SearchState::Error(msg) => {
                frame.draw_text_centered(
                    font,
                    SCREEN_HEIGHT / 2 - 20,
                    theme::ERROR,
                    1.0,
                    "Search failed",
                );
                frame.draw_text_centered(
                    font,
                    SCREEN_HEIGHT / 2 + 20,
                    theme::TEXT_MUTED,
                    0.85,
                    msg,
                );
            }
        }

        // Sticky header.
        frame.fill_rect(0.0, 0.0, SCREEN_WIDTH as f32, HEADER_H as f32, theme::FIELD_BG);
        frame.draw_text_centered(font, 26, theme::TEXT_PRIMARY, 1.1, "Search");
        frame.fill_rect(
            0.0,
            HEADER_H as f32 - 1.0,
            SCREEN_WIDTH as f32,
            1.0,
            theme::TEXT_MUTED,
        );

        // Query bar (tappable text field).
        let query_rect = Rect::new(
            12.0,
            (HEADER_H + 8) as f32,
            (SCREEN_WIDTH - 24) as f32,
            (QUERY_BAR_H - 16) as f32,
        );
        frame.fill_rect(
            query_rect.x,
            query_rect.y,
            query_rect.w,
            query_rect.h,
            theme::FIELD_BG,
        );
        let display = if self.query.is_empty() {
            "Tap to search…"
        } else {
            self.query.as_str()
        };
        let text_color = if self.query.is_empty() {
            theme::TEXT_MUTED
        } else {
            theme::TEXT_PRIMARY
        };
        frame.draw_text(
            font,
            (query_rect.x as i32) + 12,
            (query_rect.y as i32) + 24,
            text_color,
            1.0,
            display,
        );
        let query_clicked = button_invisible(query_rect, &mut self.query_btn, ctx, interactive);
        if query_clicked {
            let _ = ime.open(
                "Search",
                ImeMode::Default,
                TextBoxMode::Default,
                256,
                &self.query,
            );
        }

        // Mode bar (People | Posts) — labels are tappable, with the
        // same effect as d-pad LEFT/RIGHT.
        let mode_y = HEADER_H + QUERY_BAR_H;
        frame.fill_rect(
            0.0,
            mode_y as f32,
            SCREEN_WIDTH as f32,
            MODE_BAR_H as f32,
            theme::BACKGROUND,
        );
        let half_w = SCREEN_WIDTH / 2;
        for (i, (mode, label)) in [(Mode::People, "People"), (Mode::Posts, "Posts")]
            .iter()
            .enumerate()
        {
            let x = (i as i32) * half_w;
            let active = *mode == self.mode;
            let color = if active {
                theme::TEXT_PRIMARY
            } else {
                theme::TEXT_MUTED
            };
            let scale = 1.0;
            let (tw, th) = frame.measure_text(font, scale, label);
            let tx = x + (half_w - tw) / 2;
            let ty = mode_y + (MODE_BAR_H + th) / 2 - 4;
            frame.draw_text(font, tx, ty, color, scale, label);
            if active {
                frame.fill_rect(
                    x as f32,
                    (mode_y + MODE_BAR_H - 3) as f32,
                    half_w as f32,
                    3.0,
                    theme::ACCENT,
                );
            }
            let tab_rect = Rect::new(
                x as f32,
                mode_y as f32,
                half_w as f32,
                MODE_BAR_H as f32,
            );
            let state = match mode {
                Mode::People => &mut self.people_btn,
                Mode::Posts => &mut self.posts_btn,
            };
            if button_invisible(tab_rect, state, ctx, interactive) && !active {
                requested_mode = Some(*mode);
            }
        }
        // Mode-bar bottom separator.
        frame.fill_rect(
            0.0,
            (mode_y + MODE_BAR_H - 1) as f32,
            SCREEN_WIDTH as f32,
            1.0,
            theme::TEXT_MUTED,
        );

        // Apply mode change request from either d-pad or mode-bar tap.
        if let Some(new_mode) = requested_mode {
            if new_mode != self.mode {
                self.mode = new_mode;
                if !self.query.trim().is_empty() {
                    self.dispatch_search(ctx);
                } else {
                    self.row_heights.clear();
                    self.selected_idx = 0;
                    self.scroll_y = 0.0;
                }
            }
        }

        // Tab bar.
        if let Some(target) = self.tab_bar.render(frame, font, ctx) {
            return ScreenAction::SwitchTab(target);
        }

        // Tap detection on result rows.
        if !ctx.touches.is_empty() {
            // Exclude the bottom tab-bar band so content taps don't fall
            // through the bar (which is drawn on top and handles them).
            let touches: Vec<_> = ctx
                .touches
                .iter()
                .filter(|t| t.y < VIEWPORT_BOTTOM)
                .map(|t| (t.x, t.y))
                .collect();
            let mut tap_action: Option<SearchTapAction> = None;
            let mut y_probe = RESULTS_TOP - self.scroll_y as i32;
            match &self.state {
                SearchState::Actors(actors) => {
                    for (actor, &row_h) in actors.iter().zip(self.row_heights.iter()) {
                        let row_bottom = y_probe + row_h;
                        if row_bottom > RESULTS_TOP && y_probe < VIEWPORT_BOTTOM {
                            let row_rect = Rect::new(
                                0.0,
                                y_probe as f32,
                                SCREEN_WIDTH as f32,
                                row_h as f32,
                            );
                            if touches.iter().any(|&(x, y)| row_rect.contains(x, y)) {
                                tap_action = Some(SearchTapAction::OpenProfile(
                                    actor.handle.as_str().to_string(),
                                ));
                                break;
                            }
                        }
                        y_probe += row_h;
                    }
                }
                SearchState::Posts(posts) => {
                    for (post, &row_h) in posts.iter().zip(self.row_heights.iter()) {
                        let row_bottom = y_probe + row_h;
                        if row_bottom > RESULTS_TOP && y_probe < VIEWPORT_BOTTOM {
                            let author_rect = Rect::new(
                                0.0,
                                (y_probe + ROW_PAD_Y) as f32,
                                TEXT_LEFT as f32,
                                AVATAR_SIZE as f32,
                            );
                            if touches.iter().any(|&(x, y)| author_rect.contains(x, y)) {
                                tap_action = Some(SearchTapAction::OpenProfile(
                                    post.post.author.handle.as_str().to_string(),
                                ));
                                break;
                            }
                            // Video-embed region → OpenVideo. Takes
                            // precedence over quote and body fallbacks.
                            if let Some(target) = crate::embeds::video_in_embed(
                                post.post.embed.as_ref(),
                                post.post.author.did.as_ref(),
                            ) {
                                if let Some((ey, eh)) = crate::embeds::embed_rect(
                                    frame, font, post, y_probe, ctx.emoji,
                                ) {
                                    let er = Rect::new(
                                        TEXT_LEFT as f32,
                                        ey as f32,
                                        (SCREEN_WIDTH - TEXT_LEFT - ROW_PAD_X) as f32,
                                        eh as f32,
                                    );
                                    if touches.iter().any(|&(x, y)| er.contains(x, y)) {
                                        tap_action = Some(SearchTapAction::OpenVideo(target));
                                        break;
                                    }
                                }
                            }
                            // Quote-embed region (if any) → OpenThread of
                            // the quoted post. Checked before the body
                            // fallback.
                            if let Some(quote_uri) =
                                crate::embeds::quote_uri_in_embed(post.post.embed.as_ref())
                            {
                                if let Some((ey, eh)) = crate::embeds::embed_rect(
                                    frame,
                                    font,
                                    post,
                                    y_probe,
                                    ctx.emoji,
                                ) {
                                    let er = Rect::new(
                                        TEXT_LEFT as f32,
                                        ey as f32,
                                        (SCREEN_WIDTH - TEXT_LEFT - ROW_PAD_X) as f32,
                                        eh as f32,
                                    );
                                    if touches.iter().any(|&(x, y)| er.contains(x, y)) {
                                        tap_action = Some(SearchTapAction::OpenThread(quote_uri));
                                        break;
                                    }
                                }
                            }
                            let body_rect = Rect::new(
                                TEXT_LEFT as f32,
                                y_probe as f32,
                                (SCREEN_WIDTH - TEXT_LEFT) as f32,
                                row_h as f32,
                            );
                            if touches.iter().any(|&(x, y)| body_rect.contains(x, y)) {
                                tap_action =
                                    Some(SearchTapAction::OpenThread(post.post.uri.clone()));
                                break;
                            }
                        }
                        y_probe += row_h;
                    }
                }
                _ => {}
            }
            if let Some(action) = tap_action {
                return match action {
                    SearchTapAction::OpenProfile(handle) => ScreenAction::Push(Box::new(
                        ProfileScreen::new(Arc::clone(&self.client), Some(handle)),
                    )),
                    SearchTapAction::OpenThread(uri) => ScreenAction::Push(Box::new(
                        ThreadScreen::new(Arc::clone(&self.client), uri),
                    )),
                    SearchTapAction::OpenVideo(target) => ScreenAction::Push(Box::new(
                        crate::video_player::VideoPlayerScreen::new(
                            Arc::clone(&self.client),
                            target.did,
                            target.cid,
                        ),
                    )),
                };
            }
        }

        ScreenAction::None
    }

    fn handle_worker_response(&mut self, resp: WorkResponse) {
        match resp {
            WorkResponse::SearchActors(Ok(batch)) => {
                self.row_heights.clear();
                self.state = SearchState::Actors(batch.actors);
            }
            WorkResponse::SearchActors(Err(e)) => {
                self.state = SearchState::Error(e);
            }
            WorkResponse::SearchPosts(Ok(batch)) => {
                self.row_heights.clear();
                let posts: Vec<FeedViewPost> = batch.posts.into_iter().map(post_to_feed).collect();
                self.state = SearchState::Posts(posts);
            }
            WorkResponse::SearchPosts(Err(e)) => {
                self.state = SearchState::Error(e);
            }
            WorkResponse::Image { url, .. } => {
                self.inflight_avatars.remove(&url);
            }
            _ => {}
        }
    }

    fn top_level(&self) -> Option<TopLevel> {
        Some(TopLevel::Search)
    }

    fn control_hints(&self) -> Vec<(&'static str, &'static str)> {
        vec![("D-pad", "People / Posts")]
    }
}

enum SearchTapAction {
    OpenProfile(String),
    OpenThread(String),
    OpenVideo(crate::embeds::VideoTarget),
}

fn post_to_feed(post: PostView) -> FeedViewPost {
    FeedViewPostData {
        post,
        reason: None,
        reply: None,
        feed_context: None,
        req_id: None,
    }
    .into()
}

fn draw_actor_row(
    frame: &mut Frame,
    font: &Font,
    actor: &ProfileView,
    y_top: i32,
    row_h: i32,
    ctx: &UiCtx,
    is_selected: bool,
) {
    if is_selected {
        frame.fill_rect(
            0.0,
            y_top as f32,
            SCREEN_WIDTH as f32,
            row_h as f32,
            theme::FIELD_BG,
        );
        frame.fill_rect(0.0, y_top as f32, 3.0, row_h as f32, theme::ACCENT);
    }

    let avatar_x = ROW_PAD_X;
    let avatar_y = y_top + ROW_PAD_Y;
    let url_thumb = actor.avatar.as_deref().map(avatar_thumbnail_jpeg);
    let painted_real = match url_thumb.as_deref() {
        Some(url) => match ctx.texture_cache.get(url) {
            Some(tex) => {
                let sx = AVATAR_SIZE as f32 / tex.width().max(1) as f32;
                let sy = AVATAR_SIZE as f32 / tex.height().max(1) as f32;
                frame.draw_texture_scale(tex, avatar_x as f32, avatar_y as f32, sx, sy);
                true
            }
            None => false,
        },
        None => false,
    };
    if !painted_real {
        frame.fill_rect(
            avatar_x as f32,
            avatar_y as f32,
            AVATAR_SIZE as f32,
            AVATAR_SIZE as f32,
            placeholder_color(actor.handle.as_str()),
        );
    }
    let mask = if is_selected {
        ctx.avatar_mask_field
    } else {
        ctx.avatar_mask
    };
    if let Some(mask_tex) = mask {
        let sx = AVATAR_SIZE as f32 / mask_tex.width().max(1) as f32;
        let sy = AVATAR_SIZE as f32 / mask_tex.height().max(1) as f32;
        frame.draw_texture_scale(mask_tex, avatar_x as f32, avatar_y as f32, sx, sy);
    }

    let display = actor
        .display_name
        .as_deref()
        .filter(|s| !s.is_empty())
        .unwrap_or_else(|| actor.handle.as_str());
    let handle_str = format!("@{}", actor.handle.as_str());
    let text_x = TEXT_LEFT;
    frame.draw_text(font, text_x, y_top + ROW_PAD_Y + 18, theme::TEXT_PRIMARY, 1.0, display);
    frame.draw_text(font, text_x, y_top + ROW_PAD_Y + 42, theme::TEXT_MUTED, 0.85, &handle_str);

    frame.fill_rect(
        0.0,
        (y_top + row_h - 1) as f32,
        SCREEN_WIDTH as f32,
        1.0,
        theme::FIELD_BG,
    );
}

fn button_invisible(rect: Rect, state: &mut ButtonState, ctx: &UiCtx, enabled: bool) -> bool {
    let pressed_now = ctx.touches.iter().any(|t| rect.contains(t.x, t.y));
    let clicked = enabled && state.pressed_last && !pressed_now && ctx.touches.is_empty();
    state.pressed_last = pressed_now;
    clicked
}

fn placeholder_color(handle: &str) -> Color {
    const PALETTE: [Color; 8] = [
        Color::rgb(0xF8, 0x9A, 0x9A),
        Color::rgb(0xF8, 0xC1, 0x9A),
        Color::rgb(0xF8, 0xE8, 0x9A),
        Color::rgb(0x9A, 0xF8, 0xA0),
        Color::rgb(0x9A, 0xE0, 0xF8),
        Color::rgb(0x9A, 0xA0, 0xF8),
        Color::rgb(0xC4, 0x9A, 0xF8),
        Color::rgb(0xF8, 0x9A, 0xE0),
    ];
    let mut h: u32 = 2166136261;
    for b in handle.bytes() {
        h = h.wrapping_mul(16777619) ^ b as u32;
    }
    PALETTE[(h as usize) % PALETTE.len()]
}
